class ConversationsController < ApplicationController
  def index
    ...
    # Fetch all the creators at once to perform better
    creators = User.where(id: @one_to_ones.map(&:creator_id).compact)

    @one_to_ones.each do |o|
      o.viewer = current_user
      o.creator = creators.detect{ |c| o.creator_id == c.id }
    end
