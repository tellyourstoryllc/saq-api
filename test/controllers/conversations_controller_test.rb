...
describe ConversationsController do
  describe "GET /conversations" do
    ...
    it "must return 1-1s but not their unseen messages if the 1-1 is pending" do
      member = FactoryGirl.create(:registered_user, last_public_story_unblurred: false)
      current_user.update(one_to_one_privacy: 'avatar_image')

      i = AvatarImage.new(user: member)
      i[:image] = 'test.png'
      i.save!
      ...
      result_must_include 'one_to_one', o.id, {'object_type' => 'one_to_one', 'id' => o.id,
                                               'last_message_at' => m.created_at, 'last_seen_rank' => nil,
                                               'last_deleted_rank' => nil, 'hidden' => false}
